...
void loop() 
{
    ...
    TemperatureAbsolute = NISTmilliVoltsToDegCKtype(EmfThermalCouple + EmfColdJunction);
...
float convertADCvalueToVoltage(int ADCValue, float refVoltage)
{
	...
	// force int to float to avoid loose
    inputVoltage = (float)ADCValue * refVoltage / 1023.0;
    return inputVoltage;
}
