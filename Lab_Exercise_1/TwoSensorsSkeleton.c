...
float convertADCToVoltage(int ADCValue, float refVoltage)
{
    float inputVoltage = 0;
    inputVoltage = (float)ADCValue * refVoltage / 1024;
    return inputVoltage;
}

/* Write a function to convert degrees K to degrees C
Call it from the main() function below */
float convertDegreeKToC(float degreeInK)
{
    return (degreeInK - 273.15);
}
...
int main()
{
    ...
    int inputADCValue;
    printf("input test adc: \n");
    scanf("%d", &inputADCValue);
    float ADCVoltage = convertADCToVoltage(inputADCValue, VRef);
    // Calculate thermistor temperature in degrees C
    RThermistor = 10.0 * 3.3 / ADCVoltage - 10.0;
    float tempThemistor = convertDegreeKToC(1.0 / (1.0/298.15+ 1.0/3975.0*log(RThermistor/10)));
    // Calculate thermocouple temperature in degrees C
    float EtcReal = (ADCVoltage - 0.35)/54.4;
    float Ecoldjunction = NISTdegCtoMilliVoltsKtype(tempThemistor);
    float Tabsolute = NISTmilliVoltsToDegCKtype(EtcReal + Ecoldjunction);
    // Output results
