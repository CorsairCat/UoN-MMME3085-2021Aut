...
#include <PID_v1.h> // include PID class
...
// initial global PID objects
PID myPID(&storedMeasuredPosnFromEncoder, &percentDutyCycle, &positionSetPoint, Kp, Ki, Kd, DIRECT);
...
void setup() 
{
	...
	// set the set point to zero
	positionSetPoint = 0.0;
	// initial the myPID object
...
void controlLoop()
{
  	...
  	// ercentDutyCycle = Kp * posnError;
	// Replace the old Propotional controller to PID controller in Class
	myPID.Compute();
